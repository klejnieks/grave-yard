define(["lib/asi/asi"], function(asi) {
	...
	var bind = function(el, ev, callback) {
		...
		asi.bind.call(el, ev, callback);

		return function() {
			asi.unbind.call(el, ev, callback);
		};
	},
	...
	slice = [].slice,
	paramReplacer = /\{([^\}]+)\}/g,
	special = asi.getObject("$.event.special", [asi]) || {},
	...
	delegate = function(el, selector, ev, callback) {
		asi.delegate.call(el, selector, ev, callback);
		return function() {
			asi.undelegate.call(el, selector, ev, callback);
		};
	},
	...
	binder = function(el, ev, callback, selector) {
		return selector ? delegate(el, asi.trim(selector), ev, callback) : bind(el, ev, callback);
	},

	basicProcessor;

	var Control = asi.Control = asi.Construct({
		...
		// classes, like `asi.Construct`, and will cache functions that are action functions (see `_isAction`)
		...
		_shifter: function(context, name) {
			...
			return function() {
				context.called = name;
				return method.apply(context, [this.nodeName ? asi.$(this) : this].concat(slice.call(arguments, 0)));
			};
		},

		// ## asi.Control._isAction
		//
		...
		_action: function(methodName, options) {
			...
			// options, run `asi.sub` to replace the action template `{}` with values from the `options`
			...
			if (options || !paramReplacer.test(methodName)) {
				var convertedName = options ? asi.sub(methodName, this._lookup(options)) : methodName;
				if (!convertedName) {
					return null;
				}

				var arr = asi.isArray(convertedName),
		...
		// ## asi.Control.processors
		//
		// An object of `{eventName : function}` pairs that Control uses to
		// hook up events automatically.
		processors: {},

		// ## asi.Control.defaults
		...
		setup: function(element, options) {
			...
			this.element = asi.$(element);

			if (pluginname && pluginname !== 'asi_control') {
			...
			debugger;
			arr = asi.data(this.element, 'controls');
			if (!arr) {
				arr = [];
				asi.data(this.element, 'controls', arr);
			}
			arr.push(this);

			// The `this.options` property is an Object that contains configuration data
			// passed to a control when it is created (`new asi.Control(element, options)`)
			//
			// The `options` argument passed when creating the control is merged with `asi.Control.defaults`
			// in [asi.Control.prototype.setup setup].
			//
			...
			if(element && typeof element === "string") {
				this.render.call(this, this.element, this.options);
			}
		...
		on: function(el, selector, eventName, func) {
			...
			if (!el) {
				...
				var cls = this.constructor,
				    ...
				    destroyCB = asi.Control._shifter(this, "destroy"),
				    funcName,
				    ready;
				...
				asi.bind.call(element, "removed", destroyCB);
				bindings.user.push(function(el) {
					asi.unbind.call(el, "removed", destroyCB);
				});
			...
			if( typeof func === 'string') {
				debugger;
				func = asi.Control._shifter(this, func);
			}
		...
		destroy: function() {
			...
			controls = asi.data(this.element, "controls");
			controls.splice(asi.inArray(this, controls), 1);

			asi.trigger(this, "destroyed");
	...
	// when called.
	var processors = asi.Control.processors;
	basicProcessor = function(el, event, selector, methodName, control) {
		return binder(el, event, asi.Control._shifter(control, methodName), selector);
	};
